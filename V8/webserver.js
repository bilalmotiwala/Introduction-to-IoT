...
io.sockets.on('connection', function(socket) {
	...
	newvalue[0] = pfio.digital_read(0);
	newvalue[1] = pfio.digital_read(1);
	newvalue[2] = pfio.digital_read(2);
	newvalue[3] = pfio.digital_read(3);
	...
	function tctbutton1()
	{
		if(newvalue[0] != check[0])
		{
			if(newvalue[0] == 1)
			{
				console.log("Switch 1 is ON!");
				socket.emit('TCTBUTTON1', constone);
				check[0] = newvalue[0];
			}
			else
			{	
				console.log("Switch 1 is OFF!");
				socket.emit('TCTBUTTON1', constzero);
				check[0] = newvalue[0];
			}
		}
	};
